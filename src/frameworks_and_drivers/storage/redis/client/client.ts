...
export const connect = (clientKey: string, port: number, host: string, redisPassword?: string) => {
 ...
 * @param {string} host 
 */
const createClient = (port: number, host: string, key: string, redisPassword?: string) => {
    ...
    client.on("error", (err: any) => Logger.debug(`error redis server ${err.message}`));
